(ns intel-hip.main
  "Kotodama WASM entrypoint for the Intel_hip clean-room actor (L4) — Clojure port.
  ...
  facts (`intel_hip.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "intel_hip")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"intel_hip.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "intel_hip-compat" :tier tier :entities entities} 200])
...
(defn start! [] :intel_hip-compat/ready)
